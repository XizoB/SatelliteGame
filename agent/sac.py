...
class SAC(object):
    ...
    def update_iq(self, batch, expert_conf, offline, online, step):
        obs, next_obs, action, reward, done, is_expert, is_agent = batch

        losses = self.update_critic_iq(obs, action, reward, next_obs, done, is_agent)
        actor_alpha_losses = self.update_actor_and_alpha_iq(batch, expert_conf, offline, online)
    ...
    def update_critic_iq(self, obs, action, reward, next_obs, done, is_agent):

        with torch.no_grad():
            next_action, log_prob, _ = self.actor.sample(next_obs)

            target_Q = self.critic_target_1(next_obs, next_action)
        ...
        critic_loss = F.mse_loss(current_Q[is_agent], target_Q[is_agent])
        ...
        return {'train/sac_critic': critic_loss.item()}

    def update_actor_and_alpha_iq(self, batch, expert_conf, offline, online):
        ...
        if offline:
            # 离线更新策略网络，只使用专家的状态
            actor_loss = (self.alpha.detach() * log_prob - actor_Q_1)[is_expert].mul(expert_conf).mean()
        elif online:
            actor_loss = (self.alpha.detach() * log_prob - actor_Q_1)[is_agent].mean()
        else:
            # 在线更新策略网络，使用专家的状态与智能体的状态一起更新
            actor_loss = (((self.alpha.detach() * log_prob - actor_Q_1)[is_expert].mul(expert_conf)
    ...
    def detached_infer_v(self, state):
        ...
        v = self.detached_getV(state).squeeze()
        return v
