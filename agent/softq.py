...
class SoftQ(object):
    def __init__(self, num_inputs, action_dim, batch_size, args):
        ...
        self.q_net = hydra.utils.instantiate(agent_cfg.critic_cfg, args=args, device=self.device).to(self.device)
        self.target_net = hydra.utils.instantiate(agent_cfg.critic_cfg, args=args, device=self.device).to(self.device)
    ...
    def update(self, replay_buffer, logger, step):
        ...
        losses = self.update_critic(obs, action.unsqueeze(0).view(-1,1), reward, next_obs, done, logger, step)
